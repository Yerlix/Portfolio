...
function yer_dropdown_add_blank_item( $items, $args ) {
...
add_filter( 'wp_nav_menu_items', 'yer_dropdown_add_blank_item', 10, 2 );

/**
 * Remove empty options created in the sub levels output
 */
function yer_dropdown_remove_empty_items( $items, $args ) {
	if ( isset( $args->walker ) && is_object( $args->walker ) && method_exists( $args->walker, 'is_dropdown' ) )
		$items = str_replace( "<option></option>", "", $items );
	return $items;
}
add_filter('wp_nav_menu_items', 'yer_dropdown_remove_empty_items', 10, 2 );
...
if (is_admin()) {
	 ...
	 */
	function yer_inject_archives_menu_meta_box() {
		add_meta_box('ale-nav-advanced', __( 'Advanced Navigation', 'yerlix' ), 'yer_wp_nav_menu_archives_meta_box', 'nav-menus', 'side', 'default');
	}
	add_action( 'admin_head-nav-menus.php', 'yer_inject_archives_menu_meta_box');

	/**
	 * Add Advanced Navigation Meta Box
	 */
	function yer_wp_nav_menu_archives_meta_box() {
...
/**
	...
	* @return array
	*/
function yer_archive_menu_filter($items, $menu, $args) {
...
add_filter( 'wp_get_nav_menu_items', 'yer_archive_menu_filter', 10, 3 );
