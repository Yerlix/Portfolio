...
$yer_mail_from = new Aletheme_WP_Mail_From();
 ...
 * @throws Exception 
 */
function yer_send_contact($data) {
	$return = true;
	...
	try {
		...
		do_action('yer_contact_form_send', $data);
 ...
 * @param array $data 
 */
function yer_contact_email_send($data) {
	global $yer_mail_from;
	try {
		...
		$yer_mail_from->setName($data['name'])->setEmail($data['email'])->addFilters();
...
add_action('yer_contact_form_send', 'yer_contact_email_send');
