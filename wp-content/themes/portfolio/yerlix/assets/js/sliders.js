jQuery(document).ready(function ($) {
	...
	function get_thumb_url(url, width, height) {
		return yerlix_sliders_js.timurl + '?src=' + url  +'&w=' + width + '&h=' + height + '&q=100';
	}

	$.fn.yerlix_edit_slider = function(){
		function create_slide_item(k) {
			return $(
				'<div class="slide" id="yerlix-slide-' + k + ' " data-id="0">' + 
		...
		function add_slide_events(slide) {
			...
			slide.find("input[name='image']").bind('change', function(e, triggered){
				...
				if(path) {
					...
					$('<img />').load(function(){
						...
						self.addClass("err");
						slide.find('.image').removeClass('uploaded').find('img').attr('src', yerlix_sliders_js.assetsurl + '/images/blank.gif');						
			...
			slide.find('a.delete').click(function(e){
				...
				$.post(yerlix_sliders_js.ajaxurl, {
					'action':'delete_slide',
					'id':id
				}, function(data){
		...
		$(this).each(function(){
			...
			var entries = $('#yerlix-slides-sortable');
			...
			$('#yerlix-add-new-slide-button').click(function(e){
				e.preventDefault();
			...
			$('#yerlix-create-slider').click(function(e){
				$(this).parents('form').submit();
			});
			
			$('#yerlix-slider-delete').click(function(e){
			...
			var shortcode = $('#yerlix-slider-info .shortcode');
			
			if (shortcode.length) {
				var shortcode_input = $('#yerlix-slider-shortcode');
				var generate_shortcode = function() {
					shortcode_input.val(yerlix_generate_slider_shortcode(shortcode.data('slug'), shortcode.find('select')));
				}
			...
			var hide_area = $('#yerlix-slider-info-hide');
			if (hide_area.length) {
				...
				if (!hide_area_visible) {
					$('#yerlix-slider-info').hide();
				}
				hide_area.prepend(hide_link);
				hide_link.click(function(e){
					e.preventDefault();
					$('#yerlix-slider-info').slideToggle('fast', function(){
						var info = $(this);
	...
	$('#yerlix-edit-slider-page').yerlix_edit_slider();
	
	$.fn.yerlix_manage_sliders = function(){
		$(this).each(function(){
