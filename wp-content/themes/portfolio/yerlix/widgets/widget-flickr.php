...
class Aletheme_Flickr_Widget extends WP_Widget 
{
	...
	{
		/* Widget settings. */
		$widget_ops = array( 'classname' => 'yerlix-flickr', 'description' => 'Display a Flickr Photostream' );
		...
		$this->cache_key = 'yerlixflickrcache';
	}
	...
	public function widget($args, $instance) 
	{
		...
		?>
		<ul class="yerlix-flickr-widget cf">
			<?php foreach ($pix as $item) : ?>
				<li class="picture">
					<a href="<?php echo $item['url']?>" title="<?php echo $item['title']?> <?php _e('by', 'yerlix')?> <?php echo $item['author_name']?>" rel="external">
						<img src="<?php echo $item['thumb']?>" alt="<?php echo $item['title']?> <?php _e('by', 'yerlix')?> <?php echo $item['author_name']?> " />
					</a>
				</li>
			<?php endforeach; ?>
		</ul>
