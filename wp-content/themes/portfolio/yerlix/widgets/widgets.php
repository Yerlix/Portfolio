 ...
 * Register Sidebars
 */
function yer_init_sidebars() {
	foreach (yerlix_get_sidebars() as $sidebar) {
		register_sidebar(
				array(
					'id' => 'yerlix-' . sanitize_title($sidebar),
					'name' => $sidebar,
					'description' => $sidebar,
...
add_action('init', 'yer_init_sidebars');

/**
 * Register Theme Widgets 
 */
function yer_init_widgets() {
...
add_action('widgets_init', 'yer_init_widgets');
...
class Aletheme_WidgetImageField 
{
	...
	function __construct($widget = null, $image_id = 0) {
		...
		if ($uri && in_array($file, array('widgets.php')) && is_admin()) {
			...
			wp_enqueue_script('widgetimagefield', YERLIX_URL . '/assets/js/widgets.js', array('jquery', 'jquery-ui-core', 'thickbox', 'media-upload'), false, true);
