...
class Aletheme_Instagram_Widget extends WP_Widget 
{
	...
	{
		/* Widget settings. */
		$widget_ops = array('classname' => 'yerlix-instagram', 'description' => __('Displays latest instagrams.', 'yerlix'));
		...
		$this->cache_key = 'yerlixinstagramcache';
	}
	...
	public function widget($args, $instance) {
		...
		if(isset($instance['access_token'])) {
			...
			if(!empty($images)) {
				...
				?>
				<ul class="yerlix-instagram-widget cf">
	...
	public function form($instance ) {
		...
		?>
		<p>
			<label for="<?php echo $this->get_field_id('title'); ?>"><?php _e('Title:', 'yerlix'); ?></label>
			<input id="<?php echo $this->get_field_id('title'); ?>" name="<?php echo $this->get_field_name('title'); ?>" value="<?php echo $instance['title']; ?>" class="widefat" />
		</p>
		<?php if(!isset($instance['access_token'])): ?>
			<p>
				<label for="<?php echo $this->get_field_id('login'); ?>"><?php _e('Instagram username:', 'yerlix'); ?></label>
				<input id="<?php echo $this->get_field_id('login'); ?>" name="<?php echo $this->get_field_name('login'); ?>" type="text" value="" class="widefat" />
			</p>
			<p>
				<label for="<?php echo $this->get_field_id('pass'); ?>"><?php _e('Password:', 'yerlix'); ?></label>
				<input id="<?php echo $this->get_field_id('pass'); ?>" name="<?php echo $this->get_field_name('pass'); ?>" type="password" class="widefat" />
			</p>
		<?php else: ?>
			<p>
				<label for="<?php echo $this->get_field_id('hashtag'); ?>"><?php _e('Show latest public instagrams with following hashtag (without "#"; if empty, will show your recent instagrams):', 'yerlix'); ?></label>
			...
			<p>
				...
				<label for="<?php echo $this->get_field_id('logoutbutton'); ?>"><?php _e('Logged in as: ', 'yerlix'); echo $instance['login']; ?></label>
				<a id="<?php echo $this->get_field_id('logoutbutton'); ?>" class="button-secondary"><?php _e('Logout from Instagram', 'yerlix'); ?></a>
				<script>
