 ...
 * @return mixed 
 */
function yer_option($name, $default = false) {
	echo yer_get_option($name, $default);
}
function yer_filtered_option($name, $default = false, $filter = 'the_content') {
	echo apply_filters($filter, yer_get_option($name, $default));
}
function yer_get_option($name, $default = false) {
	$name = 'yer_' . $name;
	if (false === $default) {
		$options = yerlix_get_options();
 ...
 * @param mixed $post_id 
 */
function yer_meta($key, $single = true, $post_id = null) {
	echo yer_get_meta($key, $single, $post_id);
}
/**
 * Find meta for post
 * @param string $key
 ...
 * @param mixed $post_id 
 */
function yer_get_meta($key, $single = true, $post_id = null) {
	if (null === $post_id) {
		$post_id = get_the_ID();
	}
	$key = 'yer_' . $key;
 ...
 * @param mixed $post_id 
 */
function yer_filtered_meta($key, $filter = 'the_content', $post_id = null) {
	echo apply_filters($filter, yer_get_meta($key, true, $post_id));
}
 ...
 * @param int $isCat 
 */
function yer_permalink($system, $isCat = false) {
    echo yer_get_permalink($system, $isCat);
}
/**
 ...
 * @return string
 */
function yer_get_permalink($system, $isCat = 0)  {
    if ($isCat) {
 ...
 */
function yer_excerpt() {
    echo yer_get_excerpt();
}
/**
 ...
 */
function yer_first_category() {
    $cat = yer_get_first_category();
	if (!$cat) {
		echo '';
		return;
	}
    echo '<a href="' . yer_get_permalink($cat->cat_ID, true) . '">' . $cat->name . '</a>';
}
/**
 * Parse first post category
 */
function yer_get_first_category() {
 ...
 * @return integer 
 */
function yer_get_random_number() {
 ...
 * @return array 
 */
function yer_get_google_webfonts()
{
	return array(
        'ABeeZee' => 'ABeeZee',
        'Abel' => 'Abel',
 ...
 * @return array
 */
function yer_get_safe_webfonts() {
	return array(
		'Arial'				=> 'Arial',
...
function yer_get_typo_transforms() {
	return array(
		'none'      => 'None',
		'uppercase'	=> 'UPPERCASE',
		'lowercase'	=> 'lowercase',
...
function yer_get_typo_variants() {
	return array(
		'normal'      => 'normal',
 ...
 * Get custom RSS url
 */
function yer_get_rss() {
    $rss_url = yer_get_option('feedburner');
 ...
 * Display custom RSS url
 */
function yer_favicon() {
    $favicon = yer_get_option('favicon');
    return $favicon ? $favicon : THEME_URL . '/yerlix/assets/favicon.ico';
}
 ...
 * @param string $name
 */
function yer_part($slug, $name = null) {
 ...
 * @param type $title 
 */
function yer_page_title($title) {
	echo yer_get_page_title($title);
}
function yer_get_page_title($title) {
 ...
 * @return array 
 */
function yer_archives_get_by_year($year = "") {
	global $wpdb;
 ...
 * @return array 
 */
function yer_archives_get_years() {
	global $wpdb;
 ...
 * @return type 
 */
function yer_archives_get_months() {
 ...
 * Display Archives 
 */
function yer_archives($delim = '&nbsp;/&nbsp;') {
    $year = null;
    ?>
    <div class="ale-archives cf">
		<h4 class="a"><?php _e('Archives', 'yerlix');?></h4>
        <?php
            $months = yer_archives_get_months();
            $archives = yer_archives_get_by_year();
        ?>
        <div class="year">
        ...
        <div class="months">
            <?php foreach ($archives as $archive) : ?>
                <?php
                    ...
                    $year = $archive->year;
                    $y_archives = yer_archives_get_by_year($archive->year);
                ?>
 ...
 */
function yer_add_ajax_action($tag, $function_to_add, $priority = 10, $accepted_args = 1) {
 ...
 * @return string 
 */
function yer_contact7_form($content) {
	$matches = array();
 ...
 * @return string
 */
function yer_remove_contact7_form($content) {
...
function yerlix_enqueue_comment_reply() {
...
add_action( 'wp_enqueue_scripts', 'yerlix_enqueue_comment_reply' );

/**
 * Remove HTML attributes from comments if is Socha Comments Selected
 */
if(yer_get_option('comments_style') == 'wp'){
