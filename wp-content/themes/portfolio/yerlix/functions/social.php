 ...
 * @param int $limit
 */
function yer_tweets($limit = 1) {
    $messages = yer_get_tweets(yer_get_twitter_name(), $limit);
    ...
    foreach ( $messages as $message ) {
        ...
        $msg = yer_twitter_hyperlinks($msg);
        $msg = yer_twitter_users($msg);
 ...
 * @return array
 */
function yer_get_tweets($username, $limit = 1) {
    if (!$username) {
        return false;
    }
 ...
 * @return string 
 */
function yer_twitter_hyperlinks($text) {
 ...
 * @return string 
 */
function yer_twitter_users($text) {
 ...
 * @return string
 */
function yer_get_twitter_name() {
    preg_match('~([^/]+)$~si', yer_get_option('twi'), $matches);
    return $matches[1];
}
 ...
 * @param string $type 
 */
function yer_share($type = 'fb') {
    echo yer_get_share($type);
}
 ...
 * @return string
 */
function yer_get_share($type = 'fb', $permalink = false, $title = false) {
    if (!$permalink) {
 ...
 * @return string
 */
function yer_get_twitter() {
	return yer_get_option('twi') ?  'http://twitter.com/#!/' . yer_get_option('twi') : '';
}
function yer_twitter() {
	echo yer_get_twitter();
}