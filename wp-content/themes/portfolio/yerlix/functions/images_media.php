 ...
 * @return string
 */
function yer_parse_first_image($content = null) {
    if (!$content) {
 ...
 * @return string
 */
function yer_remove_first_image($content = null) {
    if (!$content) {
        $content = get_the_content();
    }
 ...
 * @return string 
 */
function yer_remove_images($content = null) {
    if (!$content) {
        $content = get_the_content();
    }
 ...
 * @return string
 */
function yer_wrap_images($content = null) {
    if (!$content) {
        $content = get_the_content();
    }
 ...
 * @return string
 */
function yer_get_all_images($content = null) {
    if (!$content) {
        $content = get_the_content();
    }
 ...
 * @return string
 */
function yer_clear_images($content)
{
 ...
 * @return string 
 */
function yer_fix_image_margings($x=null, $attr, $content) {
...
add_filter('img_caption_shortcode', 'yer_fix_image_margings', 10, 3);
 ...
 * @return array
 */
function yer_get_attached_images($id = null, $show_hidden = true) {
    if (!$id) {
        $id = get_the_ID();
    }
 ...
 */
function yer_get_first_attached_image($id = null, $show_hidden = true) {
    if (!$id) {
        $id = get_the_ID();
    }
 ...
 * @return string 
 */
function yer_display_first_attached_image($id = null, $size = 'large', $show_hidden = true)
{
	$image = yer_get_first_attached_image($id, $show_hidden);
 ...
 * @return string
 */
function yer_get_featured_image_src($post_id, $size  = 'thumbnail') {
	if (!$post_id) {
		$post_id = get_the_ID();
	}
 ...
 * @return string 
 */
function yer_get_first_attached_image_src($post_id, $size = 'thumbnail', $show_hidden = true) {
	if (!$post_id) {
		$post_id = get_the_ID();
	}
	$image = yer_get_first_attached_image($post_id, $show_hidden);
 ...
 */
function yer_get_attachment_id($url) {
 ...
 * @param mixed $post_id 
 */
function yer_media_meta($key, $media_id) {
	echo yer_get_media_meta($key, $media_id);
}
/**
 ...
 * @param mixed $post_id 
 */
function yer_get_media_meta($key, $media_id) {
	$key = '_yer_' . $key;
 ...
 * @return string
 */
function yer_create_youtube_player($media_source = '', $width = 640, $height = 360, $allow_autoplay = 1) {
	if (preg_match('%(?:youtube\.com/(?:user/.+/|(?:v|e(?:mbed)?)/|.*[?&]v=)|youtu\.be/)([^"&?/ ]{11})%i', $media_source, $matches)) {
		/* Give player a unique ID */
		$player_id = 'ytplayer_' . $matches[1] . '_' . yer_get_random_number();
		...
		$output = __('Sorry that seems to be an invalid <strong>YouTube</strong> URL. Please check it again.', 'yerlix');
	}
 ...
 * @return string
 */
function yer_create_vimeo_player($media_source = '', $width = 640, $height = 360, $allow_autoplay = 1) {
	if (preg_match('~^http://(?:www\.)?vimeo\.com/(?:clip:)?(\d+)~', $media_source, $matches)) {
		/* Give player a unique ID */
		$player_id = 'player_' . $matches[1] . '_' . yer_get_random_number();
		$color = yer_get_option('primary_2');
		$video_color = ( 1 == yer_get_option('enable_styles') && $color ) ? ltrim($color, '#') : '252A31';
		...
		$output = __('Sorry that seems to be an invalid <strong>Vimeo</strong> URL. Please check it again. Make sure there is a string of numbers at the end (e.g. http://vimeo.com/1234567).', 'yerlix');
	}
	return $output;
}
 ...
 * function of yer_create_vimeo_player() or yer_create_youtube_player()
 *
 * @return string
 */
function yer_create_wp_embed_player( $media_source = '', $width = 640, $height = 360, $allow_autoplay = 1 ) {
...
function yer_get_embed_video($url) {
 ...
 * @return string 
 */
function yer_embed_thumbnail($url, $echo = true) {
	$data = yer_get_embed_video($url);
	if ($echo) {
 ...
 * @return string
 */
function yer_lazyload_images($content) {
 ...
 * @return type
 */
function yer_bulk_lazyload_images($content) {
