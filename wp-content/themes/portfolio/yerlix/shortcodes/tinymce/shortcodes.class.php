...
class yer_shortcodes
{
	...
	function formate_shortcode()
	{
		...
		if( isset( $yer_shortcodes[$this->popup]['child_shortcode'] ) )
			$this->has_child = true;
		
		if( isset( $yer_shortcodes ) && is_array( $yer_shortcodes ) )
		{
			// get shortcode config stuff
			$this->params = $yer_shortcodes[$this->popup]['params'];
			$this->shortcode = $yer_shortcodes[$this->popup]['shortcode'];
			$this->popup_title = $yer_shortcodes[$this->popup]['popup_title'];
			
			// adds stuff for js use			
			$this->append_output( "\n" . '<div id="_yer_shortcode" class="hidden">' . $this->shortcode . '</div>' );
			$this->append_output( "\n" . '<div id="_yer_popup" class="hidden">' . $this->popup . '</div>' );
			
			if( isset( $yer_shortcodes[$this->popup]['no_preview'] ) && $yer_shortcodes[$this->popup]['no_preview'] )
			{
				//$this->append_output( "\n" . '<div id="_yer_preview" class="hidden">false</div>' );
			...
			{
				// prefix the fields names and ids with yer_
				$pkey = 'yer_' . $pkey;
			...
			{
				// set child shortcode
				$this->cparams = $yer_shortcodes[$this->popup]['child_shortcode']['params'];
				$this->cshortcode = $yer_shortcodes[$this->popup]['child_shortcode']['shortcode'];
			
				// popup parent form row start
				$prow_start  = '<tbody>' . "\n";
				$prow_start .= '<tr class="form-row has-child">' . "\n";
				$prow_start .= '<td><a href="#" id="form-child-add" class="button-secondary">' . $yer_shortcodes[$this->popup]['child_shortcode']['clone_button'] . '</a>' . "\n";
				$prow_start .= '<div class="child-clone-rows">' . "\n";
				
				// for js use
				$prow_start .= '<div id="_yer_cshortcode" class="hidden">' . $this->cshortcode . '</div>' . "\n";
